...
'''                                    
def eval_ner(data_res):
    gold_list = data_res['gold_str'].tolist()
    pred_list = data_res['pred_str'].tolist()
...
def get_fuzzy_metrics(df):

    # gold_str: stringa, gold entities separate da "; " (anche lo spazio). Se non ci sono, la stringa "none".
    # pred_str: stringa, pred entities separate da "; " (anche lo spazio). Se non ci sono, la stringa "none".

    eval_ner_res = eval_ner(df)
    scores = calc_score(eval_ner_res)
    return scores
