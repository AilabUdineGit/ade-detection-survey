...
from ade_detection.models.bert_wrapper_binary import Bert_wrapper_binary
...
class TrainerBinary(object):
    ...
    def __init__(self, task: Task):
        ...
        if task.train_mode == TRAIN_MODE.VALIDATION:
            self.train_dataset = task.split.to_tensor_dataset(task.split.train, binary=True)
            self.validation_dataset = task.split.to_tensor_dataset(task.split.validation, binary=True)
        elif task.train_mode == TRAIN_MODE.TESTING:
            self.train_dataset = task.split.to_tensor_dataset(task.split.train + task.split.validation, binary=True)
            self.validation_dataset = task.split.to_tensor_dataset(task.split.test, binary=True)
        ...
        model_classes = {
            ARCHITECTURE.BERT_WRAPPER: Bert_wrapper_binary,
