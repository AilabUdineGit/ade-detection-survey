...
class Trainer(object):
    ...
    def __init__(self, task: Task):
        ...
        if task.train_mode == TRAIN_MODE.TESTING:
        ...
        if task.train_mode == TRAIN_MODE.JUST_TESTING:
            self.train_dataset = None
            self.validation_dataset = task.split.to_tensor_dataset(task.split.test)
        
        if self.validation_dataset == None:
            raise Exception("Wrong train mode provided: {task.train_mode}")
    ...
    def init_val_df(self, validation_dataset):
        #print([d for d in validation_dataset]) 
        val_df_index = [int(d[-1]) for d in validation_dataset]
        
        n_epochs = 1 if self.TEST_ONLY else self.task.train_config.epochs
        columns = ['numeric_id', 'input_ids', 'input_mask', 'gold_labels'] 
        columns += [f"preds_{i+1}" for i in range(n_epochs)]
            
        val_df = pd.DataFrame(index=val_df_index, columns=columns)
    ...
    def do_train_val(self, train_dataloader, validation_dataloader, val_df):
        ...
        if not self.TEST_ONLY:
            ...
            for epoch_i in tqdm(range(self.task.train_config.epochs), desc="Training"):
                # training
    ...
    def test_model(self, validation_dataloader, val_df, epoch):
        ...
        for batch in validation_dataloader:
            ...
            for i, pred_tensor in enumerate(preds):
                ...
                mask = b_input_mask[i].tolist()
                real_pred = pred[:(mask.index(0) if 0 in mask else len(pred))]
