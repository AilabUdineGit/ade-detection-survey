...
class BayerImporter(BaseImporter):

    '''
        Importer script for the Bayer dataset
    '''
    ...
    def encode_dataset(self, dataset):
        ...
        for _, row in tqdm(dataset.iterrows(), total=len(dataset)):
            docs = list(filter(lambda x: x.external_id == row.doc_id, documents))
            if len(docs) == 0:
                doc = Document(external_id = row.doc_id, 
                               text = row.text, 
                               corpus = CORPUS.BAYER
    ...
    def load_dataset(self):
        ...
        dtype = {'doc_id': object, 'text': object}

        df = pd.read_csv("assets/datasets/bayer_dataset/bayer_dataset.csv", sep=',', dtype=dtype) 
